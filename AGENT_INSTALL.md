...
3. 阅读 `install.sh`、`scripts/patch-theme.mjs`、`scripts/install-theme.mjs` 和 `theme/qq2008-notify.js`，确认没有本机用户路径或未说明的外部上传。
...
7. 安装后从目标 `app.asar` 验证：
   ...
   - `renderer/index.html` 包含 `qq2008-notify.js`
   - `renderer/assets/qq2008-skin.css` 存在
   - `renderer/assets/qq-penguin-user.png` 存在
   - `renderer/assets/qq-message.wav` 和 `renderer/assets/qq-failure.wav` 存在
